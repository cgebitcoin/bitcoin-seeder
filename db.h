...
protected:
  ...
  bool Get_(CServiceResult &ip,
            int &wait); // get an IP to test (must call Good_, Bad_, or Skipped_
  ...
  void Good_(const CService &ip, int clientV, std::string clientSV, int blocks,
             uint64_t services); // mark an IP as good (must have been returned
                                 // by Get_)
  void Bad_(const CService &ip,
            int ban); // mark an IP as bad (and optionally ban it) (must have
                      // been returned by Get_)
...
public:
  ...
  void GetStats(CAddrDbStats &stats) {
    SHARED_CRITICAL_BLOCK(cs) {
      ...
      stats.nAge =
          ourId.empty() ? 0 : (time(NULL) - idToInfo[ourId[0]].ourLastTry);
    }
  }
  ...
  //   CAddrInfo[n]
  //   banned
  // acquires a shared lock (this does not suffice for read mode, but we assume
  // that only happens at startup, single-threaded) this way, dumping does not
  // interfere with GetIPs_, which is called from the DNS thread
  IMPLEMENT_SERIALIZE(
      ({
        int nVersion = 0;
        READWRITE(nVersion);
  ...
  void Good(const CService &addr, int clientVersion,
            std::string clientSubVersion, int blocks, uint64_t services) {
    CRITICAL_BLOCK(cs)
