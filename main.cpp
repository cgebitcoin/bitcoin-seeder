...
public:
  ...
  CDnsSeedOpts()
      : nThreads(96), nDnsThreads(4), ip_addr("::"), nPort(53), nP2Port(0),
        nMinimumHeight(0), mbox(NULL), ns(NULL), host(NULL), tor(NULL),
        fUseTestNet(false), fWipeBan(false), fWipeIgnore(false),
        ipv4_proxy(NULL), ipv6_proxy(NULL), magic(NULL) {}
  ...
  void ParseCommandLine(int argc, char **argv) {
    ...
    while (1) {
      ...
      int c = getopt_long(argc, argv,
                          "s:h:n:m:t:a:p:d:o:i:k:w:b:q:x:", long_options,
                          &option_index);
      if (c == -1)
        break;
      switch (c) {
    ...
    if (filter_whitelist.empty()) {
      ...
      filter_whitelist.insert(NODE_NETWORK | NODE_WITNESS |
                              NODE_COMPACT_FILTERS); // x49
      filter_whitelist.insert(NODE_NETWORK | NODE_WITNESS |
                              NODE_P2P_V2); // x809
      filter_whitelist.insert(NODE_NETWORK | NODE_WITNESS | NODE_P2P_V2 |
                              NODE_COMPACT_FILTERS);                     // x849
      ...
      filter_whitelist.insert(NODE_NETWORK_LIMITED | NODE_WITNESS |
                              NODE_COMPACT_FILTERS); // x448
      filter_whitelist.insert(NODE_NETWORK_LIMITED | NODE_WITNESS |
                              NODE_P2P_V2); // xc08
      filter_whitelist.insert(NODE_NETWORK_LIMITED | NODE_WITNESS |
                              NODE_P2P_V2 | NODE_COMPACT_FILTERS); // xc48
      filter_whitelist.insert(NODE_NETWORK_LIMITED | NODE_WITNESS |
                              NODE_BLOOM); // x40c
    }
...
extern "C" void *ThreadCrawler(void *data) {
  ...
  do {
    ...
    for (int i = 0; i < ips.size(); i++) {
      ...
      res.fGood =
          TestNode(res.service, res.nBanTime, res.nClientV, res.strClientV,
                   res.nHeight, getaddr ? &addr : NULL, res.services);
    }
    db.ResultMany(ips);
    db.Add(addr);
  } while (1);
...
extern "C" int GetIPList(void *thread, char *requestedHostname, addr_t *addr,
                         int max, int ipv4, int ipv6);
...
public:
  ...
  void cacheHit(uint64_t requestedFlags, bool force = false) {
    ...
    if (force ||
        ...
        (thisflag.cacheHits * thisflag.cacheHits * 20 > thisflag.cache.size() &&
...
extern "C" int GetIPList(void *data, char *requestedHostname, addr_t *addr,
                         int max, int ipv4, int ipv6) {
  ...
  unsigned int maxmax =
      (ipv4 ? thisflag.nIPv4 : 0) + (ipv6 ? thisflag.nIPv6 : 0);
  if (max > size)
    max = size;
  if (max > maxmax)
    max = maxmax;
  ...
  do {
      ...
      count++;
    {
      ...
      fprintf(d, "# address                                        good  "
                 "lastSuccess    %%(2h)   %%(8h)   %%(1d)   %%(7d)  %%(30d)  "
                 "blocks      svcs  version\n");
      double stat[5] = {0, 0, 0, 0, 0};
      for (vector<CAddrReport>::const_iterator it = v.begin(); it < v.end();
           it++) {
        CAddrReport rep = *it;
        fprintf(d,
                "%-47s  %4d  %11" PRId64
...
int main(int argc, char **argv) {
  ...
  pthread_attr_setstacksize(&attr_crawler, 0x200000);
