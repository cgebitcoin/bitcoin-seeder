...
void SplitHostPort(std::string in, int &portOut, std::string &hostOut) {
  ...
  bool fBracketed =
      fHaveColon &&
      (in[0] == '[' &&
       in[colon - 1] == ']'); // if there is a colon, and in[0]=='[', colon is
...
bool LookupHost(const char *pszName, std::vector<CNetAddr> &vIP,
  ...
  char psz[256];
  char *pszHost = psz;
  seeder_strlcpy(psz, pszName, sizeof(psz));
...
bool GetNameProxy() { return nameproxyInfo.second != 0; }
...
bool ConnectSocketByName(CService &addr, SOCKET &hSocketRet,
  ...
  SOCKET hSocket = INVALID_SOCKET;
  CService addrResolved(CNetAddr(strDest, fNameLookup && !nameproxyInfo.second),
                        port);
...
void CNetAddr::Init() { memset(ip, 0, 16); }
...
bool CNetAddr::IsIPv6() const { return (!IsIPv4() && !IsTor() && !IsI2P()); }
...
bool CNetAddr::IsReserved() const { return IsIPv4() && (GetByte(3) >= 240); }
...
bool CNetAddr::IsValid() const {
  ...
  // addr26... so if the first length field is garbled, it reads the second
  // batch of addr misaligned by 3 bytes.
...
bool CNetAddr::IsRoutable() const {
  return IsValid() &&
         !(IsReserved() || IsRFC1918() || IsRFC3927() || IsRFC4862() ||
           (IsRFC4193() && !IsTor() && !IsI2P()) || IsRFC4843() || IsLocal());
}
...
std::vector<unsigned char> CNetAddr::GetGroup() const {
  ...
  else if (GetByte(15) == 0x20 && GetByte(14) == 0x11 && GetByte(13) == 0x04 &&
           GetByte(12) == 0x70)
    nBits = 36;
...
void CNetAddr::print() const { printf("CNetAddr(%s)\n", ToString().c_str()); }
...
int CNetAddr::GetReachabilityFrom(const CNetAddr *paddrPartner) const {
  ...
  default:
    ...
    case NET_TOR:
      return REACH_PRIVATE; // either from Tor/I2P, or don't care about our
                            // address
    }
  }
}
...
void CService::print() const { printf("CService(%s)\n", ToString().c_str()); }
